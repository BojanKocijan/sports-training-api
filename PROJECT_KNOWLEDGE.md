...
- [ ] Planned mascot roster (proposed 2026-09-17, not yet added to `mascots` — no artwork exists for any of them yet): Tiger, Bear, Panda, Fox, Wolf, Rabbit, Monkey, Dog, Cat, Red panda, Eagle. Lion is the only mascot seeded today; each new animal needs both a `mascots` row and full `mascot_avatars` coverage (8 jersey colors × however many age stages get art) before it's usable, same shape as Lion's 16 basketball rows.

---

## Changelog

- **2026-09-19** — #65 Parent-code reads no longer put the trainer passcode in the URL: `GET /players/:id/parent-code?passcode=...` was removed and replaced with `POST /players/:id/parent-code/read` using `{ passcode }` in the JSON body. Responses that reveal a parent code (read and issue) send `Cache-Control: no-store`; the OpenAPI contract and API/UI tests enforce the body-only credential flow.
- **2026-09-17** — #51 Expanded `mascot_stages` from 4 bands to 7 to match the actual mascot art plan: `baby` (0-8) and `child` (8-10) unchanged; `teen` narrowed to 10-12; `adult` renamed to `grownup` at 18+; three new stages fill the 12-18 gap — `puber` (12-14), `mid-teens` (14-16), `adolescence` (16-18). Added matching `group_templates` rows U16/U18/grownup (all `coming_soon`, same as U12/U14). Only `teen` has any artwork so far (the existing white/orange Lion images, reused as a stopgap). Resolution (`GET /mascots/avatars?groupId=`) only reads a group's `min_age`, never its `max_age` — a custom group spanning two bands (e.g. 10-14) resolves to whichever stage its `min_age` lands in, not a blend of both.
- **2026-09-17** — #49 `GET /mascots/avatars?groupId=` — resolves a group's `sport_id` + `min_age` (per #45/#72, not through `group_templates`) to a `mascot_stages` row, then queries `mascot_avatars` on `(sport_id, stage)`. Age is the resolution key, not the group's name/template. No match (group not found, or `min_age` unset) returns `[]`, not an error. Also extended #47's seed to cover `stage='baby'` with the same 8 images (no dedicated baby art yet — without this, U8 groups resolved to zero rows since `group_templates` maps U8→`baby` but only `child` had seeded art).
- **2026-09-17** — #47 `mascot_avatars` gained a `jersey_color` column (nullable, unique key now `(mascot_id, sport_id, stage, jersey_color)`) and its first real rows: the 8 existing Leon basketball jersey images, seeded as `stage='child'` since that's the only stage with art today. Why: stage and color are independent axes — stage tracks how mature the mascot looks as a kid moves up age groups, color is the player's own pick — and the existing 8 jersey colors are genuinely different illustrations, not one pose recolored, so mascot_avatars needed a real column to hold them. Open question flagged, not resolved: stage × color is multiplicative (4 stages × 8 colors = 32 images per mascot per sport if every future stage gets the full palette).
...
- **2026-09-18** — #57/#59 Shark added as the second mascot, same pose as the lion: a `mascots` row plus `stage='baby'` boy/girl `mascot_avatars` rows, and a new `mascot_avatars.eyes_highlights_url` column (the unblended white eye-shine dots — per row because they move with the eyes; lion baby rows backfilled). The shark's eyes sit wider/higher than the lion's and the girl's lower than the boy's, so each row carries its own eye mask, `eyes_layout` and highlights. Boy placement is the designer's (Figma node `4029:2824`, 410,330 333x164); the girl's (405,349 334x152) was derived from her measured eye highlights and checked at zoom — if she looks off, re-pull from Figma. Jersey/number/logo boxes are shared across mascots since the pose is identical. Rollout order matters: deploy sports-training-ui first (it carries the shark images), then run the SQL.
- **2026-09-18** — #57/#59 `mascot_avatars` gained `gender`, `jersey_mask_url`/`jersey_layout`, `eyes_mask_url`/`eyes_layout`, `number_layout`, `logo_layout` — dynamic multiply-blend art for `stage='baby'`, replacing that stage's 8 shared-stopgap jersey_color rows with 2 real rows (one per gender). Jersey(+shorts+shoes) and eyes now recolor client-side via CSS `mix-blend-mode: multiply` against a grayscale-ready base pose, instead of one fully-baked image per jersey color — `jersey_mask_url`/`eyes_mask_url` are URL templates with a `{color}` placeholder the client fills in (8 jersey colors, 3 eye colors: blue/green/brown), not per-color DB rows. Every mask asset + layout box traced directly in Figma (`J9dSOUC5az7RoMJlNegRtr`, node `4008:346`), not eyeballed. Only `stage='baby'` (U8) got this treatment — `child`/`teen` etc. keep their existing per-color-image rows untouched. `mane`/`fur`/`sweatband` stay fixed/painted for now, per issue #57's phased scope — only jersey and eyes are dynamic so far. Open: no per-player `gender` or `eye_color` field exists yet, so `JerseyGraphic.tsx` currently defaults both rather than letting the player choose (see sports-training-ui's own changelog).
